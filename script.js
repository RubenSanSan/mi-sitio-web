...
(async () => {
    ...
    const battleMenuToggle = document.getElementById("battle-menu-toggle"), tradeMenuToggle = document.getElementById("trade-menu-toggle"), craftingMenuToggle = document.getElementById("crafting-menu-toggle");
    ...
    const state = {
        ...
        recipes: [], buildingMap: new Map(), specialPorts: [],
    ...
    function handleClick(e) { if (!state.isClick) return; const mouseX = (e.offsetX - state.offsetX) / state.scale, mouseY = (e.offsetY - state.offsetY) / state.scale; let clickedPort = null; for (const port of state.ports) { if (!port.coordinates) continue; const [x, y] = port.coordinates; if (Math.hypot(mouseX - x, mouseY - y) < 16) { clickedPort = port; break; } } if (clickedPort) { showPortPanel(clickedPort); } else { hidePortPanel(); } }
    ...
    function panToPort(port) { if (!port.coordinates) return; const targetScale = 0.5; state.scale = targetScale; state.offsetX = (canvas.width / 2) - (port.coordinates[0] * targetScale); state.offsetY = (canvas.height / 2) - (port.coordinates[1] * targetScale); showPortPanel(port); battleMenu.classList.add('hidden'); tradeMenu.classList.add('hidden'); craftingMenu.classList.add('hidden'); statsMenu.classList.add('hidden'); }
    ...
    // --- Lógica de Inteligencia de Batalla (Rediseñada) ---
    function populateBattleIntelligence() {
        const now = new Date();
        const SPANISH_NATION_ID = 2;
        const dateOptions = { day: '2-digit', month: 'short' };

        // 1. Alertas de Raid (sin cambios)
        state.raidAlertPorts = state.ports.filter(p => {
            ...
            return raidDate.getTime() > (now.getTime() - (2 * 60 * 60 * 1000));
        });
        ...
        if (spanishDefenses.length > 0) {
            spanishDefensesList.innerHTML = spanishDefenses.map(item =>
                `<li data-port-id="${item.port.Id}">
                    ...
                    <span class="port-date defense">Activa hasta: ${item.cooldownEnd.toLocaleDateString('es-ES', dateOptions)}</span>
                </li>`
            ).join('');
        } else {
        ...
        // 3. Próximos Objetivos (Puertos enemigos vulnerables pronto)
        ...
        if (attackTargets.length > 0) {
            attackTargetsList.innerHTML = attackTargets.map(item =>
                `<li data-port-id="${item.port.Id}">
                    ...
                    <span class="port-date">Vulnerable: ${item.cooldownEnd.toLocaleDateString('es-ES', dateOptions)}</span>
                </li>`
            ).join('');
        } else {
    ...
    function buildCraftingDatabase() { state.itemTemplatesMap.forEach(item => { if (item.__type && item.__type.includes('Recipe') && item.BuildingRequirements && item.BuildingRequirements.length > 0) { state.recipes.push(item); } if (item.__type && item.__type.includes('Building')) { state.buildingMap.set(item.Id, item.Name); } }); state.specialPorts = state.ports.filter(p => p.ImprovedCraftChance || p.CanConvertResources); }
    ...
    function initializeSideMenus() {
        ...
        recipeFinderInput.addEventListener('input', (e) => {
            ...
            const results = state.recipes.filter(r => r.Name.toLowerCase().includes(searchTerm)).slice(0, 5);
            recipeFinderResults.innerHTML = results.map(recipe => {
                ...
                return `<div class="recipe-result" data-recipe-id="${recipe.Id}"><strong>${recipe.Name}</strong><div class="recipe-details"><h5>Requiere: ${buildingName}</h5><ul>${materials}</ul></div></div>`;
            }).join('');
        });
        recipeFinderResults.addEventListener('click', (e) => { const target = e.target.closest('.recipe-result'); if (target && target.dataset.recipeId) { const recipeId = parseInt(target.dataset.recipeId, 10); const recipe = state.recipes.find(r => r.Id === recipeId); if (recipe) { showMaterialLocations(recipe.Name, recipe.FullRequirements || []); } } });
        ...
        specialPortsList.addEventListener('click', (e) => { const target = e.target.closest('li'); if (target && target.dataset.portId) { const port = state.ports.find(p => p.Id === target.dataset.portId); if (port) panToPort(port); } });
        ...
        // Event listeners para las nuevas listas de batalla
        spanishDefensesList.addEventListener('click', (e) => { const target = e.target.closest('li'); if (target && target.dataset.portId) { const port = state.ports.find(p => p.Id === target.dataset.portId); if (port) panToPort(port); } });
        attackTargetsList.addEventListener('click', (e) => { const target = e.target.closest('li'); if (target && target.dataset.portId) { const port = state.ports.find(p => p.Id === target.dataset.portId); if (port) panToPort(port); } });
    ...
    // --- INICIALIZACIÓN ---
    function initialize() {
        battleMenuToggle.addEventListener('click', (e) => { e.stopPropagation(); battleMenu.classList.toggle('hidden'); tradeMenu.classList.add('hidden'); craftingMenu.classList.add('hidden'); statsMenu.classList.add('hidden'); battleMenuToggle.classList.toggle('active'); tradeMenuToggle.classList.remove('active'); craftingMenuToggle.classList.remove('active'); statsToggle.classList.remove('active'); state.selectedTradeRoute = null; });
        tradeMenuToggle.addEventListener('click', (e) => { e.stopPropagation(); tradeMenu.classList.toggle('hidden'); battleMenu.classList.add('hidden'); craftingMenu.classList.add('hidden'); statsMenu.classList.add('hidden'); tradeMenuToggle.classList.toggle('active'); battleMenuToggle.classList.remove('active'); craftingMenuToggle.classList.remove('active'); statsToggle.classList.remove('active'); state.selectedTradeRoute = null; });
        craftingMenuToggle.addEventListener('click', (e) => { e.stopPropagation(); craftingMenu.classList.toggle('hidden'); battleMenu.classList.add('hidden'); tradeMenu.classList.add('hidden'); statsMenu.classList.add('hidden'); craftingMenuToggle.classList.toggle('active'); battleMenuToggle.classList.remove('active'); tradeMenuToggle.classList.remove('active'); state.selectedTradeRoute = null; });
        statsToggle.addEventListener('click', (e) => { e.stopPropagation(); statsMenu.classList.toggle('hidden'); battleMenu.classList.add('hidden'); tradeMenu.classList.add('hidden'); craftingMenu.classList.add('hidden'); statsToggle.classList.toggle('active'); battleMenuToggle.classList.remove('active'); tradeMenuToggle.classList.remove('active'); craftingMenuToggle.classList.remove('active'); state.selectedTradeRoute = null; });
        mapContainer.addEventListener('click', () => { battleMenu.classList.add('hidden'); tradeMenu.classList.add('hidden'); craftingMenu.classList.add('hidden'); statsMenu.classList.add('hidden'); battleMenuToggle.classList.remove('active'); tradeMenuToggle.classList.remove('active'); craftingMenuToggle.classList.remove('active'); statsToggle.classList.remove('active'); state.selectedTradeRoute = null; });
        ...
        closeMaterialPanelButton.addEventListener('click', () => { materialPanel.classList.add('hidden'); });
...
>>>>>>> df70756fbc9b12c3911a3ed39b553a67cbff529e
